...
import cn.lioyan.codecs.EmptyDocValuesProducer;
import cn.lioyan.search.DocIdSetIterator;
import cn.lioyan.util.BitSet;
import cn.lioyan.util.Counter;
import cn.lioyan.util.FixedBitSet;
import cn.lioyan.util.packed.PackedLongValues;

import java.io.IOException;

import static cn.lioyan.search.DocIdSetIterator.NO_MORE_DOCS;

/**
 * {@link NumericDocValuesWriter}
 *  用于临时存储 number类的 文档id与文档值
 *  后期会被封装为 {@link NumericDocValues} 用于建立列式索引
 *
 *  其中 id通过{@link  DocsWithFieldSet}
 *  内容通过{@link PackedLongValues}
 *
 *
 *  其中通过flush 方法，将数据刷新到文件
 *
 *
 * @author com.lioyan
 * @date 2023/6/14  10:44
 */
public class NumericDocValuesWriter extends DocValuesWriter<NumericDocValues>
{
    ...
    NumericDocValuesWriter(FieldInfo fieldInfo, Counter iwBytesUsed) {
        pending = PackedLongValues.deltaPackedBuilder(COMPACT);
